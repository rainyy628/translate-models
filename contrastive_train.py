...
def train(config,model,train_iter,dev_iter,test_iter):
    ...
    for epoch in range(config.num_epochs):
        ...
        for i,(trains) in enumerate(train_iter):
            ...
            labels=torch.tensor(trains[2]).to(config.device)
            outputs=model(trains[0],trains[1])
            torch.cuda.empty_cache()
...
def evaluate(config,model,dev_iter):
    ...
    with torch.no_grad():
        for inputs in dev_iter:
            labels=torch.tensor(inputs[2]).to(config.device)
            outputs=model(inputs[0],inputs[1])
            #计算准确度
            true = labels.data.cpu()
            predic = torch.max(outputs.data, 1)[1].cpu()
            dev_acc = metrics.accuracy_score(true, predic)
            acc_total+=dev_acc
            torch.cuda.empty_cache()
            loss=F.cross_entropy(outputs,labels)
            loss_total=loss_total+loss
    length=len(dev_iter)
    return loss_total/length,acc_total/length
...
#分类矩阵对角线元素置-1
def diagonal_reset(matrix,size):
    for i in range(size):
        matrix[i,i] = -1.0
    return matrix