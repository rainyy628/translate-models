...
import ContrastiveSentenceBert as myModel
import numpy as np
import torch
import pandas as pd
...
config_all=torch.load('/root/code/yaoyao/mb/saved_dict_contrastive/ContrastiveBert_3allaug_2layers_075.ckpt')
...
def get_noted(str_list):
    ...
    for str_index in range(str_nums):
        ...
        for i in range(len(cutting_word_list)):
            ...
            if times==1:
                ...
                word_site=len(''.join(cutting_word_list[0:i]))+len(str_before)+1
                # sites_new,str_new=get_sentence_cut(word_site,str)
                word_sites_list.append(word_site)
                s_composed_list.append(long_str)
                meanings_index_list=[]
                #计算目标句子和字典句子中对应字的相似性
                sims=[]
                #随机获取字典中不同释义对应的句子
                ...
                best_choice=sims.index(max(sims))
    ...
    return word_list,word_meaning_list,sentence_list
...
for file_name in file_names:
    ...
    for line in passage:
        ...
        w_list,meaning_list,s_list=get_noted(str_list)
...
df_dict=pd.DataFrame({'sentence':sentences,'words':words,'words_meaning':meanings})
#df_dict.to_csv(obj_path+'/guwen_大学_sentencebert_con.csv',index=False)
